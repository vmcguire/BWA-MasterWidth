# BWA-MasterWidth — Read this before doing anything
...
can clone, browse, and reason about BWA-MasterWidth on its own without
seeing the rest of the BWA monorepo.

All real work on BWA-MasterWidth happens inside the BWA monorepo at:

```
~/Developer/bwa-fx-ecosystem/plugins/masterwidth/
    dsp/           ← authoritative DSP implementation (THE source of truth)
    ui/            ← UI surface (Metal-rendered)
    plugin/        ← VST3 wrapper → builds BWA-MasterWidth.vst3
    manifest.json  ← name, controls, license tier, ecosystem benefits
```

If you've been asked to "work on BWA-MasterWidth," **navigate to the
...
1. **Standalone VST3** (this repo's release artifact) — `BWA-MasterWidth.vst3`
...
- **Category:** C (Spectral DSP Processor, Dual-Surface (post-merge — master))
- **Tech tree tier:** Tier 3
- **Role:** Master width.
- **Pain attacked:** Mono-comp meter + per-band width + per-band M/S X-ray.
- **Scope verdict:** master-only
- **Band tier behavior:** Per-band width resolution scales with tier.

Full row in
[`MARKET-PAIN-AND-ATTACKS.md`](https://github.com/vmcguire/BWA-Architecture/blob/main/docs/MARKET-PAIN-AND-ATTACKS.md)
— search for `BWA-MasterWidth`.
...
2. **[`BWA-Architecture/docs/USER-STORY-MAP.md`](https://github.com/vmcguire/BWA-Architecture/blob/main/docs/USER-STORY-MAP.md)**
   — where BWA-MasterWidth sits in the bedroom-producer journey
   (Stage 8 — MASTER).
3. **[`BWA-Architecture/docs/PLAN-2026-Q2.md`](https://github.com/vmcguire/BWA-Architecture/blob/main/docs/PLAN-2026-Q2.md)**
   — current build plan + repo inventory. Find BWA-MasterWidth in §9 and
...
4. **[`BWA-Architecture/docs/MARKET-PAIN-AND-ATTACKS.md`](https://github.com/vmcguire/BWA-Architecture/blob/main/docs/MARKET-PAIN-AND-ATTACKS.md)**
   — the row for BWA-MasterWidth. Internalize the pain + the attack before
...
- ❌ Committing to this repo. Go to the monorepo (`plugins/masterwidth/`).
...
- ❌ Adding new shared/* modules without checking if the capability folds
  ...
  Methodology Rule 3."
- ❌ Skipping the BWA-MasterWidth row in MARKET-PAIN-AND-ATTACKS.md. If the
  proposed work doesn't advance the BWA Attack for this plugin, push back.

## What "done" looks like

A `BWA-MasterWidth.vst3` that:
...
4. Renders the **24-band X-ray view** for its operation (the BWA visual
   signature).
7. Tiers cleanly across 3 / 6 / 12 / 24 band resolutions via the license
   document.
...
```bash
git subtree push --prefix=plugins/masterwidth \
  git@github.com:vmcguire/BWA-MasterWidth.git main
```

Then on GitHub:

1. Create a Release from `main` in this repo.
2. Attach the notarized `BWA-MasterWidth.vst3` zip.
3. Reference the monorepo commit SHA in the release notes.

---

_End of LLM onboarding for BWA-MasterWidth._
