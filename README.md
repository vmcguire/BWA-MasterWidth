...
Wideband stereo widener with always-visible mono-comp meter + per-band width (low always mono, sides only above crossover).
...
BWA-MasterWidth is a wideband stereo widener that doesn't break mono compat. Mono-compatibility meter front-and-center. Per-band width — low always mono, sides only above crossover.

X-ray showing what's added to side vs subtracted from mid per band.
...
BWA-MasterWidth ships on **two surfaces** — same DSP, different UI:
...
[Releases](https://github.com/vmcguire/BWA-MasterWidth/releases). Build from
